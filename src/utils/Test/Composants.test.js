...
const ComposantTest = ({ shorts = false }) => {
  return (
    <div id="Container-level-3">
      <div
        className={`${shorts ? "shorts" : "MoreVideos"}`}
      ...
      <button onClick={() => MoreContent(3, shorts)} id="Button-section-3">
        Bouton
      </button>
    </div>
  );
...
test("Afficher plus de contenu avec MoreContent() sans shorts", async () => {
  ...
  expect(screen.queryByTestId("TestContent")).not.toHaveStyle(
    "flex-wrap: nowrap;",
  );
  expect(
    screen.queryByRole("button", { name: /Bouton/i }),
