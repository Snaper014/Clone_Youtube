...
React.useLayoutEffect(() => {
  ...
  React.useEffect(() => {
    ...
    const isScrollAtBottom = () => {
        ...
        const lastElementVisible = new IntersectionObserver(entries => {
             ...
             ReactDOM.createRoot(div).render(
                <NewSearchs data={response} 
                  ...
                  sessionStorage.setItem("Token", JSON.stringify(response?.data?.continuation))
                  loading = false; 
                    })
                    .catch(error => {
                      console.log(error)
      ...
      WidthShorts, marginLeft, 
      MarginRight, value, setOption, setDataContext])
    ...
    if(status === 'fail'){
      return(
        <div style={{width: '100%'}}>
          <p style={{fontSize: '32px'}}>Une Erreur est survenu {error.message}</p>
        </div>
      )
    }
