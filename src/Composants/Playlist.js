              ...
              top: '11vh'
               ...
               <div style={{
                ...
                flexDirection: 'column',
                  ...
                  <ReactPlayer url={`https://www.youtube.com/watch?v=${ViedosPlaylist}`} width={"100%"} height={"500px"} className='react-player' controls />
                  <h1 style={{fontWeight: 'bolder', marginTop: '1%', fontSize: '20px', width: '100%', marginBottom: '2%'}}>{DetailVideo?.data?.title}</h1>
                  <div style={{ 
                          width: '100%', 
                          display: 'flex',
                          flexDirection: 'row',
                      ...
                      <div onClick={() => HandleChannel(DetailVideo?.data?.channelId)} style={{
                        width: '50%',
                        display: 'flex',
                        flexDirection: 'row',
                        flexWrap: 'nowrap',
                       ...
                       <div style={{marginLeft: '2%', marginRight: '5%'}}>
                          <h5 style={{fontSize: '18px'}}>
                            {DetailVideo?.data?.channelBadges === null  ? 
                               ...
                               :     
                                    <>
                               {DetailVideo?.data?.channelTitle} <span><VscVerifiedFilled /></span>
                                     </>
                            }                        
                            </h5>
                          <p style={{fontSize: '14px'}}>{DetailVideo?.data?.subscriberCountText}</p>
                        </div>
                              <button style={{
                                  display: 'flex',
                                  alignItems: 'center',
                  ...
                  style={{
                    ...
                    <h3 style={{width: '100%'}}>{DetailVideo?.data?.viewCount} vues - {DetailVideo?.data?.publishDate}</h3>   
                    <div style={{fontSize: '20px'}}>
                      {
                       DisplayDescription ? 
                            <>
                                ...
                                : <p>{DetailVideo?.data?.description.substring(0, 312)}  <span style={{marginLeft: '2%', fontWeight: '550', fontSize: '18px'}}>Plus</span></p>
               ...
               <div style={{
                 ...
                 alignItems: 'flex-end'
                    ...
                    <p>{DetailVideo?.data?.channelTitle} - {parseInt(index) + 1} / {Playlist?.data?.data?.length}</p>
                    </div>    
                    {Playlist?.data?.data?.map((element, i) => (
                    <div onClick={() => HandlePlaylist(element?.videoId, i, Identifiant)} 
                      key={i} 
                      style={{
                      width: '100%',
                      display: 'flex',
                      ...
                      cursor: 'pointer',
                      backgroundColor: `${parseInt(index) === i ? 'rgba(127,82,97,0.4)' : 'white'}`

                      }} 
                      >
                        { parseInt(index) === i ? <BsFillPlayFill style={{display: 'flex', alignSelf: 'center', fontSize: '18px'}} /> 
                              :
                          <p style={{display: 'flex', alignSelf: 'center', fontSize: '18px'}}>{i + 1}</p>
                        }
                        <div style={{width: '35%', display: 'flex', justifyContent: 'center', alignItems: 'flex-end', position: 'relative'}} onClick={() => HandleVideos(element?.videoId)}>
                            <img  style={{borderRadius: '10px', height: '15vh', width: '100%'}} src={element?.thumbnail[0]?.url} alt={index}>
                            </img>
                            <div style={{
                                 position: 'absolute',
                                 width: '35px',
                               ...
                               <p style={{fontWeight: '600', color: 'white', margin: '0.3rem', fontSize: '1em'}}>{element?.lengthText}</p>
                            </div>
                        </div>
                        <div style={{
                          display: 'flex',
